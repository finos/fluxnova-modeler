...
describe('<PrivacyPreferencesView>', function() {
  ...
  describe('functionality', function() {
    ...
    it('should default to false (opt-out) if preferences unset', function() {
    ...
    it('should set autofocus to first preference if not specified', async function() {

      // when
      render(<PrivacyPreferencesView />);

      // then
      const checkbox = screen.getByLabelText(new RegExp(PREFERENCES_LIST[0].title, 'i'));
    ...
    it('should set autofocus if specified', async function() {
      ...
      // then
      const checkbox = screen.getByLabelText(new RegExp(PREFERENCES_LIST[1].title, 'i'));
      expect(checkbox === document.activeElement).to.be.true;
    });
